...
def lnd(
    ...
    if hue is None:
        _onesample_lnd(data, x, y, hue, palette, ax, colors, line, dots, average, flipped, verbose, adtnl_space,
...
def _onesample_lnd(
    ...
    ax = sns.stripplot(data=data, x=x, y=y, palette=palette, ax=ax, dodge=True, size=size)
    for i, art in enumerate(ax.collections):
        try:
            offsets = art.get_offsets()
            average = offsets.mean(axis=0)  # first number is x (mean position), second is y (mean value)
